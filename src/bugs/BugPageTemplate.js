...
const BugPageTemplate = ({ bug, children }) => {
  ...
  const [alreadyCaught, setAlreadyCaught] = React.useState(
    bug?.name ? hasBug(bug.name) : false
  );
  
  React.useEffect(() => {
    if (bug?.name && hasBug(bug.name)) {
    ...
    <>
      ...
      <Box direction="row">
        ...
        >
          ...
          {tests.map((bugTest, index) => (
          <Box key={bugTest.label || index}
